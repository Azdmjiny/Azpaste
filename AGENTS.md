# Azpaste Screenshot 项目说明

## 项目概览

Azpaste Screenshot 是一个轻量的 macOS 截屏应用。项目使用 Swift 编写，直接基于 AppKit 构建桌面窗口和菜单栏交互，并使用 Carbon 注册全局快捷键。
...
- `Sources/AzpasteScreenshot/main.swift`：应用的全部 Swift 源码，包含窗口 UI、菜单栏、截屏流程、快捷键、权限提示、保存目录和自测参数逻辑。
...
构建并启动应用。`make run` 依赖 `app` 目标，只有 `Sources/AzpasteScreenshot/main.swift`、`Info.plist` 或 `Makefile` 更新后才会重新构建。
...
- 使用 `swiftc` 直接编译 `Sources/AzpasteScreenshot/main.swift`。
- 输出可执行文件为 `build/Azpaste Screenshot.app/Contents/MacOS/AzpasteScreenshot`。
- 目标平台为 `arm64-apple-macos13.0`。
- 链接 `AppKit` framework。
...
```sh
make -B app
codesign --verify --verbose=4 --strict build/Azpaste\ Dev.app
codesign -dvvv build/Azpaste\ Dev.app
```
...
- 选区模式下，鼠标按下记录起点，拖拽更新终点，松开后生成全局坐标矩形。
...
6. 全屏截图直接在后台线程调用截图逻辑。
7. 截图成功后保存 PNG，并通过 `NSPasteboard` 写入剪贴板。

### 全屏与选区截图

`captureScreen(rect:destination:)` 使用 `CGDisplayCreateImage` 获取显示器图像。

如果传入 `rect`：

- 先通过选区中心点找到包含该区域的显示器。
- 根据 `CGDisplayBounds` 和实际 `CGImage` 像素尺寸计算缩放比例。
- 将 AppKit 坐标中的选区转换为图片像素裁剪区域。
- 调用 `cropping(to:)` 得到裁剪后的图片。

如果没有传入 `rect`，则保存当前显示器整张截图。

### 窗口截图

`captureWindowImage(at:destination:)` 先用点击位置查找窗口 ID，再用 `CGWindowListCreateImage` 截取该窗口。

窗口查找逻辑在 `windowID(at:)` 中：

- 使用 `CGWindowListCopyWindowInfo` 获取屏幕上的窗口。
- 只考虑 layer 为 `0` 的普通窗口。
- 排除 owner name 为 `Azpaste Screenshot` 的自身窗口。
...
应用依赖 macOS 屏幕录制权限。首次截屏或权限缺失时，应用会提示用户到系统设置中允许 `Azpaste Screenshot` 录制屏幕，并调用系统 API 请求权限。
...
- 目标平台固定为 Apple Silicon 的 macOS 13 或更高版本。
...
- 修改 `Sources/AzpasteScreenshot/main.swift`、`Info.plist` 或 `Makefile` 后，下一次 `make app` 会重新构建并签名。
