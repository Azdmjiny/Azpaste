...
- `Scripts/make_icns.swift`：构建辅助脚本，用系统 Swift/AppKit 从 `Resources/app.png` 生成 macOS `.icns` 应用图标。
...
- 应用图标源文件是 `Resources/app.png`，`Makefile` 会调用 `Scripts/make_icns.swift` 生成 `Azpaste.icns`；更换应用图标时通常只需要替换这个 PNG 并重新 `make app`。
